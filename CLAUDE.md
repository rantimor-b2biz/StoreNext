...
1 article + LinkedIn post + brand visual per week (Mon ~08:30 IL) generated by
`.github/workflows/article-post-generate.yml` → `T-tools/scripts/generate_article_and_post.py`
...
Output: `O-output/W[NN]/` (final/ + process/). Topic dedup: `B-brain/topic-history.json`.
`send-post-email.yml` now only syncs Firebase mediaPlan (email moved into the script).

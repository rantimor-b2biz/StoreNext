...
| `routine-drafts` | כל דראפט תוכן שבועי. | Codex בסיום כל סבב |
| `Codex/*` | ענפי עבודה זמניים של Codex web — תשתית בלבד. | נמחקים אחרי merge |

### ⚠️ CONTENT MUST GO TO routine-drafts — NOT TO Codex/* BRANCHES

**כלל קריטי:** כל קובץ O-output/ חייב להידחף ל-`routine-drafts`. לעולם לא ל-`Codex/*`.

סשן ROUTINE פותח אוטומטית ענף `Codex/*`. זה בסדר לשינויי תשתית.
...
אם דחפת בטעות ל-`Codex/*` — העבר ידנית:
```bash
...
תשתית (AGENTS.md, A-agents, C-core) — PR מענף Codex/* → main.
...
## Content Automation (live since July 2026 — replaces the old weekly topic-proposals flow)

2 runs per week — article + LinkedIn post + brand visual, generated by
`.github/workflows/article-post-generate.yml` → `T-tools/scripts/generate_article_and_post.py --brand <storenext|meteor>`
(Researcher + web search → Copywriter article → Gatekeeper post + visual-data →
`T-tools/generate-visual.py` PNG → **everything emailed to Ran for manual upload**).

- **Monday ~08:30 IL → Meteor** (fintech division, pillar 2 only, `product_focus: Meteor`)
- **Thursday ~08:30 IL → StoreNext** (core supplier portal brand, pillars 1/3/4)

Output: `O-output/W[NN]/final/` + `process/` — same week folder for both brands.
StoreNext files keep their original unsuffixed names (`final-post.md`, `article.md`, ...)
so the Firebase mediaPlan sync (`send-post-email.yml`, triggered only by
`O-output/W*/final/final-post.md`) keeps working unchanged. Meteor files get a
`-meteor` suffix (`final-post-meteor.md`, `article-meteor.md`, `W[NN]-meteor-visual.png`)
and are email-only — not synced to the mediaPlan. Topic dedup is per-brand:
`B-brain/topic-history.json` entries carry a `brand` field.
Manual resend for either brand: `resend-week-email.yml` (or
`T-tools/scripts/send_week_email.py W[NN] --brand meteor`).
...
Do NOT use `isolation: "worktree"` when calling agents. All work goes directly on the main branch. Worktrees are a developer tool — using them in content workflows causes files to get lost in `.Codex/worktrees/`.
